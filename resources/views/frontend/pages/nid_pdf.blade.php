                            ...
                            <div class="mb-3 row d-flex justify-content-around">
                                @if ($nid_pdf->available == 1)
                                    <div class="col-5 border rounded">
                                        <label class="radio-inline mb-0 cursor-pointer " style="padding: 10px 0px;"> <input
                                                type="radio" name="type_name" value="nid_pdf" class="cursor-pointer"> NID
                                            PDF (<span class="text-danger">{{ number_format($nid_pdf->cost, 0) }}</span>TK)
                                        </label>
                                    </div>
                                @endif
                                @if ($smrt_nidPdf->available == 1)
                                    <div class="col-5 border rounded">
                                        <label class="radio-inline mb-0 cursor-pointer" style="padding: 10px 0px;"> <input
                                                type="radio" name="type_name" value="smrt_nid_pdf" class="cursor-pointer">
                                            Smart NID PDF (<span
                                                class="text-danger">{{ number_format($smrt_nidPdf->cost, 0) }}</span>TK)</label>
                                    </div>
                                @endif
                            </div>

                            <div class="mb-3">
                                <label for="" class="form-label text-dark">Select Type:</label>
                                <div class="form-group">
                                    <label class="radio-inline mr-3"> <input type="radio" name="type" value="form_no">
                                        Form NO</label>
                                    <label class="radio-inline mr-3"> <input type="radio" name="type" value="nid_no">
                                        NID NO</label>
                                    <label class="radio-inline mr-3"> <input type="radio" name="type" value="voter_no">
                                        Voter NO</label>
                                    <label class="radio-inline mr-3"><input type="radio" name="type" value="birth_no">
                                        Birth NO</label>
                            ...
                            @forelse ($orders as $order)
                                <tr>
                                    ...
                                    <td class="d-flex flex-column">
                                        <span> Option:
                                            <strong>{{ $order->type_name == 'smrt_nid_pdf' ? 'Smart NID PDF' : 'NID PDF' }}</strong></span>
                                        <span>Type:
                                            <strong>{{ $order->type == 'nid_no' ? 'NID NO' : ($order->type == 'voter_no' ? 'Voter NO' : ($order->type == 'form_no' ? 'Form NO' : 'Birth NO')) }}</strong></span>
    ...
    <script>
        $(document).ready(function() {
            $('#nid_pdf_form').on('submit', function(e) {
                ...
                $('input[type="radio"][name="type_name"]').on('change', function() {
